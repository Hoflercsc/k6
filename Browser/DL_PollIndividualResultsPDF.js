...
export default function getPollHttp() {
  ...
  group("open up poll 2214 individual results", function () { 
    const url = constants.values.POLL_GET_POLL_INDIVIDUAL_RESULTS;
    let response = http.get(url);
    common.debug("open up poll 2214 individual results > "+response.status)
  ...
  group("download poll 2214 individual results pdf", function () {
    const url = constants.values.POLL_GET_POLL_INDIVIDUAL_RESULTS_PDF;
    let response = http.get(url);
    common.debug("download poll 2214 individual results pdf > "+response.status)

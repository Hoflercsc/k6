...
export default function downloadLargeBook(token) {
  ...
  group("download book", function () {
    ...
    let response = http.get(url, buildJsonRequestHeaders(token));
    common.debug("book downloaded: status = "+response.status);

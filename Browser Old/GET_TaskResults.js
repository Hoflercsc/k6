...
export default function getTask() {
  ...
  group("go to tasks", function () {
    sleep(3);
    const url = (`https://lab-load-testing.boardeffect.com/workrooms/8205/tasks`);
    let response = http.get(url);
    console.log("go to tasks > "+response.status)
  ...
  group("get tasks 351 results", function () {
    sleep(3);
    const url = (`https://lab-load-testing.boardeffect.com/workrooms/8205/tasks/351/results`);
    let response = http.get(url);
    console.log("get tasks results > "+response.status)

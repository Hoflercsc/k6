...
export default function downloadBookHttp() {
  ...
  group("go to workroom", function () {
    let url = ('https://lab-load-testing.boardeffect.com/workrooms/8205/books');
    let response = http.get(url);
    console.log("go to workroom = " + response.status);
  ...
  group("download large book", function () {
    let url = ('https://lab-load-testing.boardeffect.com/workrooms/8205/books/11700/download');
    let response = http.get(url);
    console.log("download large book = " + response.status);

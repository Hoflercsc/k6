...
export default function downloadSurveyResultsSummaryPDF() {
  ...
  group("go to survey 2208 summary", function () {
   ...
   group("download survey results sumamry pdf", function () {
    const url = (`https://lab-load-testing.boardeffect.com/workrooms/8205/surveys/reports_generate?id=2208&report_format=pdf&report_name=survey_summary_2208`);
    let response = http.get(url);
    console.log("download survey results sumamry pdf > "+response.status)

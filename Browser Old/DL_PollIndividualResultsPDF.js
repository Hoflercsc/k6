...
export default function getPollHttp() {
  ...
  group("open up poll 2214 individual results", function () {
    sleep(3);
    // even though the link shows surveys it is actually getting the poll results 
    const url = (`https://lab-load-testing.boardeffect.com/workrooms/8205/surveys/2214/individual`);
    let response = http.get(url);
    console.log("open up poll 2214 individual results > "+response.status)
  ...
  group("download poll 2214 individual results pdf", function () {
    sleep(3);
    // even though the link shows surveys it is actually getting the poll results 
    const url = (`https://lab-load-testing.boardeffect.com/workrooms/8205/surveys/reports_generate?id=2214&report_format=pdf&report_name=survey_individual_2214-12234&user_id=12234`);
    let response = http.get(url);
    console.log("download poll 2214 individual results pdf > "+response.status)

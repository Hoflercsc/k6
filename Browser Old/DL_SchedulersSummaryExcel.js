...
export default function getSchedulersHttp() {
  ...
  group("download results summary excel", function () {
    sleep(3);
    // even though the link shows surveys it is actually download excel 
    const url = (`https://lab-load-testing.boardeffect.com/workrooms/8205/surveys/reports_generate?id=2211&report_format=xlsx&report_name=survey_summary_2211`);
    let response = http.get(url);
